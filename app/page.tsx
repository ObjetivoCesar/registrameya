...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-background selection:bg-primary/30 scroll-smooth relative overflow-hidden">
      ...
      <section className="py-24 relative overflow-hidden bg-navy">
        <div className="absolute inset-0 z-0">
          <Image
            ...
            alt="Comunidad" fill className="object-cover opacity-40 grayscale-[30%]"
          />
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          >
            {[
              { icon: Search, title: "Buscan tu oficio y sales tú", text: "Configuramos palabras clave estratégicas para que cuando busquen tu especialidad aparezcas de primero." },
              { icon: Camera, title: "Confianza al Instante", text: "Una foto profesional integrada hace que confíen en ti antes de hablarte. Te ves más pro." },
      ...
      {/* Registro Section */}
      <section id="registro" className="py-32 bg-background">
        <div className="max-w-5xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-20 text-navy uppercase font-black tracking-tighter text-4xl italic leading-none">
            Crea tu Tarjeta Ahora
          </motion.div>
          <RegisterWizard />
        </div>
      </section>
      ...
      <section id="precios" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {[
              { name: "Básico", price: "10", color: "navy/5", features: ["vCard Estratégica", "Tu Foto de Perfil", "Categoría de Búsqueda", "Historial de Llamadas", "Entrega en 1 hora"] },
              { name: "Pro", price: "20", color: "primary", features: ["Todo el Básico", "Código QR para Imprimir", "Galería de 3 Fotos", "Botón WhatsApp Directo", "Diseño Premium", "Soporte Prioritario"] }
            ].map((plan) => (
              <motion.div key={plan.name} whileHover={{ y: -10 }} className={`bg-white border-4 border-${plan.color} p-12 rounded-[40px] flex flex-col shadow-soft`}>
                <h3 className={`text-xl font-black uppercase tracking-widest mb-4 opacity-40`}>{plan.name}</h3>
                ...
                <a href="#registro" className="w-full py-5 bg-navy/5 text-navy font-black rounded-button text-center uppercase tracking-widest text-xs">Elegir {plan.name}</a>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="py-20 relative overflow-hidden bg-navy">
        <div className="absolute inset-0 z-0">
          <Image src="https://images.unsplash.com/photo-1541123437800-1bb1317badc2?auto=format&fit=crop&q=80&w=2000" alt="Footer" fill className="object-cover opacity-20 grayscale" />
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center justify-between gap-12 text-center md:text-left">
          <div className="text-2xl font-black text-white tracking-tighter"><span className="text-primary">!</span>Regístrame Ya</div>
          <p className="text-sm text-white/30 font-black uppercase tracking-widest">© 2026 · Profesionales en Acción</p>
